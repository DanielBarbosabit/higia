$(document).ready(function() {
    ...
    carrega_info_atendimento();

})

function carrega_info_atendimento(){
    $("#id_atendimento").val(info_atendimento['id_atendimento']);
    $("#inicio_atend").val(info_atendimento['inicio_atendimento']);
    $("#final_atend").val(info_atendimento['final_atendimento']);
    $("#observacoes").val(info_atendimento['observacao_atendimento']);

    valor_total = "R$ " + (parseFloat(info_atendimento['valor_total']).toFixed(2)).replace(".", ",");
    $("#total").val(valor_total);

    pagamento_efetuado = info_atendimento['pagamento_efetuado']
    if (pagamento_efetuado == 1){
        $("#pagamento_sim").prop("checked", true);
    }else{
        $("#pagamento_nao").prop("checked", true);
    }

//    Carrega subtotais de cada procedimento
    Object.keys(procedimentos).forEach(k =>{
        input_subtotal = "#valor_proc_" + procedimentos[k][0]
        if (procedimentos[k][4] > 0){
            $(input_subtotal).val(procedimentos[k][3]);
        }
    })
...
function carrega_procedimentos(){
   Object.keys(procedimentos).forEach(k =>{
    ...
    if (procedimentos[k][4] > 0){
        string_botao_minus_desabilitado = ""
    }
    else{
        string_botao_minus_desabilitado = 'disabled="disabled"'
    }
    new_proc =  '<div class="row" style="margin-top:1em;">' +
                    '<div class="col-sm-2" style="width:150px">' +
                        '<div class="input-group">' +
                            '<span class="input-group-prepend">' +
                                '<button type="button" class="btn btn-outline-secondary btn-number"' + string_botao_minus_desabilitado + 'data-type="minus" data-field="quant_proc_' + procedimentos[k][0] + '">' +
                                    '<span class="fa fa-minus"></span>' +
                                '</button>' +
                            '</span>' +
                            '<input type="text" name="quant_proc_' + procedimentos[k][0] + '" id="quant_proc_' + procedimentos[k][0] + '" class="form-control input-number" value=' + procedimentos[k][4] +' min="0" max="99">' +
